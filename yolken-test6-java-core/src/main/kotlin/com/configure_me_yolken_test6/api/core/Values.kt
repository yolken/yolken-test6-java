...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { YolkenTest6InvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw YolkenTest6InvalidDataException("`$name` is not set")
            is JsonNull -> throw YolkenTest6InvalidDataException("`$name` is null")
            else -> throw YolkenTest6InvalidDataException("`$name` is invalid, received $this")
        }
